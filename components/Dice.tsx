...
const Dice: React.FC<TimerButtonProps> = ({ telegramId, diceUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`dice_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`dice_${telegramId}`, "true");
    ...
    try {
      ...
      localStorage.removeItem(`dice_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
