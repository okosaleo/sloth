...
const Shiba: React.FC<TimerButtonProps> = ({ telegramId, shibaUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`shiba_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`shiba_${telegramId}`, "true");
    ...
    try {
      ...
      window.location.href = shibaUrl;
    } catch (error) {
      console.error("Error updating points:", error);

      // Optionally: revert click state if needed
      localStorage.removeItem(`shiba_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
