...
const ShaCommunity: React.FC<TimerButtonProps> = ({ telegramId, telegramUrl}) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = Cookies.get(`sha_${telegramId}`);  // Ensure proper key formatting
  ...
  const handleClick = async () => {
    ...
    Cookies.set(`sha_${telegramId}`, "true", { expires: 365, path: "" }); 
    ...
    try {
      ...
      Cookies.remove(`sha_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false)
    }
