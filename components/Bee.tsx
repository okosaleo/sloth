...
const Bee: React.FC<TimerButtonProps> = ({ telegramId, beeUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`bee_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`bee_${telegramId}`, "true");
    ...
    try {
      ...
      window.location.href = beeUrl;
    } catch (error) {
      console.error("Error updating points:", error);

      // Optionally: revert click state if needed
      localStorage.removeItem(`bee_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
