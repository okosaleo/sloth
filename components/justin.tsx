...
const Justin: React.FC<TimerButtonProps> = ({ telegramId, justinUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`jus_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`jus_${telegramId}`, "true");
    ...
    try {
      ...
      window.location.href = justinUrl;
    } catch (error) {
      console.error("Error updating points:", error);

      // Optionally: revert click state if needed
      localStorage.removeItem(`jus_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
