...
const Money: React.FC<TimerButtonProps> = ({ telegramId, moneyUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`mon_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`mon_${telegramId}`, "true");
    ...
    try {
      ...
      localStorage.removeItem(`mon_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
