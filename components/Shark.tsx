...
const Shark: React.FC<TimerButtonProps> = ({ telegramId, sharkUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`shark_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`shark_${telegramId}`, "true");
    ...
    try {
      ...
      window.location.href = sharkUrl;
    } catch (error) {
      console.error("Error updating points:", error);

      // Optionally: revert click state if needed
      localStorage.removeItem(`shark_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
