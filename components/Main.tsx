...
const Main: React.FC<TimerButtonProps> = ({ telegramId, mainUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = localStorage.getItem(`main_${telegramId}`);
  ...
  const handleClick = async () => {
    ...
    setHasClicked(true);
    localStorage.setItem(`main_${telegramId}`, "true");
    ...
    try {
      ...
      localStorage.removeItem(`main_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false);
    }
