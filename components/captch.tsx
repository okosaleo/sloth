...
const Community: React.FC<TimerButtonProps> = ({ telegramId, telegramUrl}) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from LocalStorage
  useEffect(() => {
    const clickedState = Cookies.get(`captch_${telegramId}`);  // Ensure proper key formatting
  ...
  const handleClick = async () => {
    ...
    Cookies.set(`captch_${telegramId}`, "true", { expires: 365, path: "" }); 
    ...
    try {
      ...
      Cookies.remove(`captch_${telegramId}`);
      setHasClicked(false);
    } finally {
      setLoading(false)
    }
