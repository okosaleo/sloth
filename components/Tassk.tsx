...
const Task: React.FC<TimerButtonProps> = ({ telegramId, taskUrl }) => {
  const [hasClicked, setHasClicked] = useState(false);
  const [loading, setLoading] = useState(false);

  // Load state from Cookies
  useEffect(() => {
    const clickedState = Cookies.get(`hasClim_${telegramId}`);  // Ensure proper key formatting
  ...
  const handleClick = async () => {
    ...
    Cookies.set(`hasClim_${telegramId}`, "true", { expires: 365, path: "" }); // Cookie persists for 1 year
